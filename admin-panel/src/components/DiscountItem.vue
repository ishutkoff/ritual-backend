<template>
	<div class="discount-item">
		<div @click="deleteDiscount()" class="discount-item__remove-btn">&times;</div>
		<div class="discount-item__picture">
			<a @click.prevent="getDiscount(discount)" href="#">
				<img v-if="discount.image" :src="`${defaultPath}/files/${discount.image}`" alt="" />
		...
		<div class="discount-item__title">
			<a @click.prevent="getDiscount(discount)" href="#">{{ discount.title }}</a>
			<p>Скидка: {{ discount.discount }}</p>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		...mapActions(['removeDiscount']),
		getDiscount(discount) {
			this.$emit('discount-click', discount)
		},
		deleteDiscount() {
			if (confirm('Вы действительно хотите удалить товар?')) {
				this.removeDiscount(this.discount._id)
			}
		},
	},
	created() {
