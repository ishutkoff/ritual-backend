<template>
	<div class="discount-edit-form">
		<div class="discount-edit-form__left">
			<label>
				<img v-if="previewImage" :src="previewImage" alt="" />
				<svg v-else version="1.0" xmlns="http://www.w3.org/2000/svg" width="416.000000pt" height="416.000000pt"
					viewBox="0 0 416.000000 416.000000" preserveAspectRatio="xMidYMid meet">
					<g transform="translate(0.000000,416.000000) scale(0.100000,-0.100000)" fill="#3e3e3e" stroke="none">
				...
				<input type="file" accept="image/gif, image/jpeg, image/png" @change="handleFileUpload($event)" />
				<div>Загрузить изображение</div>
			</label>
		</div>
		<div class="discount-edit-form__right">
			<input v-model="newDiscount.title" type="text" placeholder="Название товара" />
			<input v-model="newDiscount.discount" type="number" placeholder="Процент скидки" />
		</div>
	</div>
	<div class="discount-edit-form__footer">
...
export default {
	...
	methods: {
		...
		async sendEditForm() {
			if (this.image) {
				...
				await this.removePicture(this.newDiscount.image)
				this.newDiscount.image = res
			}
			this.newDiscount.category = this.currentCategory
			this.$emit('send-form', this.newDiscount)
		},
		handleFileUpload(e) {
	...
	computed: {
		getImageSrc() {
			return this.newDiscount.image
				? `${this.defaultPath}/files/` + this.newDiscount.image
				: ''
		},
		isEmpty() {
			if (!this.newDiscount.title || this.newDiscount.price === '') return true
			return false
		},
		getCategories() {
	...
	mounted() {
		...
		this.newDiscount = Object.assign({}, this.discount)
		this.previewImage = this.getImageSrc
		if (this.newDiscount.category) {
			this.currentCategory = this.newDiscount.category
		}
	},
}
</script>
